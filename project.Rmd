...
write_xlsx(data_us,"data_br.xlsx")
```
...
# Influence of US interest rate in BR interest rate
```{r}
data_br$observation_date <- as.POSIXct(data_br$observation_date)

merged_data <- merge(data_br, data_us, by="observation_date")

ggplot(data = merged_data, aes(x=interest_us, y=interest_brazil)) +
  geom_point() +
  ggtitle("Relationship between interest rate in Brazil and USA")+
  ylab("Interest Rate Brazil") +
  xlab("Interest Rate USA") +
  scale_y_continuous(labels = scales::percent)+
  scale_x_continuous(labels = scales::percent)
